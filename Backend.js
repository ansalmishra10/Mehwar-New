...
class Backend {
    ...
    messagesRefs = null;
    messagesRefss = null;
    // initialize Firebase Backend00000000
    constructor() {
        firebase.initializeApp({

            apiKey: "AIzaSyAOK5ik6NVNtqp9T_EMCkL4ypZ4iG6g14I",
            authDomain: "diskuss-48116.firebaseapp.com",
            databaseURL: "https://mehwar-5cd43.firebaseio.com/",
            projectId: "mehwar-5cd43",
            storageBucket: "diskuss-48116.appspot.com",
            messagingSenderId: "539135236659",
            appId: "1:539135236659:web:651c4a27eeaa2596d7a146",
            measurementId: "G-NRLT8ZR7NE"
        });
    ...
    loadMessagess(callbacks) {
        ...
        const onReceives = data => {
            ...
            this.messagesRefs.once("value").then(function (snapshot) {
                ...
                if (message.userid == GLOBAL.userID) {
                    callbacks({
                        name: message.name,
    ...
    updateMessage(callbacks) {
        ...
        var query = this.messagesRefss.orderByChild('anotherid').equalTo(GLOBAL.userID)
    ...
    loadMessages(callback) {
        ...
        var query = this.messagesRef.orderByChild('anotherid').equalTo(GLOBAL.userID)
        query.on('value', function (snapshot) {
            snapshot.forEach(function (weekSnapshot) {
                console.log(weekSnapshot.val());
    ...
    updateTyping(text) {
        this.messagesRefs = firebase.database().ref().child("chat/" + GLOBAL.bookingid + 'type');
        if (text.length == 0) {
            this.messagesRefs.update({ typinguser: false, typinganother: false, name: GLOBAL.myname, userid: GLOBAL.userID });
            //  this.messagesRefs[0].typinguser = false

        } else {
            this.messagesRefs.update({ typinguser: true, typinganother: false, name: GLOBAL.myname, userid: GLOBAL.userID });
    ...
    sendMessage(message) {
        ...
        for (let i = 0; i < message.length; i++) {
            ...
            var r = "";
            var kr = message[i].hasOwnProperty('video')
            if (kr == true) {
                r = message[i].video;
